...
|---|---|---|---|
| A — fundamentos | [00](../00-logica-javascript/README.md) | Retomar valores, decisões, repetições, funções, coleções, módulos e JSON; registrar sessões coerentes com Git. Atividades além do recorte servem como reforço. | Tentativas autorais de programas pequenos, explicação e modificação registradas. |
| B — protocolo | [01](../01-http-rest/README.md) | Observar conversas HTTP e trabalhar com um servidor HTTP pequeno, mantendo o foco em request e response. Atividades adicionais permanecem como reforço. | Requisições analisadas, servidor explicado e mudança autoral documentada. |
| C — API tipada | [03](../03-node-typescript-api/README.md) | Executar uma API em memória, estudar TypeScript estrito e acompanhar o fluxo assíncrono. Atividades além do recorte ficam para aprofundamento. | API em memória reproduzível, com tipos e comportamento assíncrono explicados. |
| D — persistência | [02](../02-modelagem-sql/README.md) | Criar um esquema pequeno, migrations simples e consultas de filtro, agregação e `JOIN`, sem ORM. Exercícios adicionais continuam como reforço. | Banco recriável e consultas fundamentais explicadas pelo estudante. |
| E — projeto demonstrável | [07](../07-testes-automatizados/README.md) e [12](../12-logs-monitoramento-deploy/README.md) | Integrar uma API de registros de expedições com persistência, testes essenciais, README e demonstração; publicar apenas quando houver opção gratuita adequada. O restante dos módulos é continuação. | Demonstração reproduzível de uma API com um recurso principal, ou publicação gratuita sem cartão quando viável. |
